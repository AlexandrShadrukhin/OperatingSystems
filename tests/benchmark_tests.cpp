...
TEST(DedupBenchmarkTest, RunsSuccessfully) {
    int ret = std::system("${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/dedup_benchmark-app 100 10000");;
    EXPECT_EQ(ret, 0) << "dedup_benchmark-app не завершился успешно";
}

TEST(SearchNameBenchmarkTest, RunsSuccessfully) {
    std::ostringstream commandStream;
    commandStream << "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/search_name_benchmark-app 2 ./ CMakeCache.txt";
